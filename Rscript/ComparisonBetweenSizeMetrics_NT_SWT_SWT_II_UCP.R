...
useCaseData <- data[c("Effort_Norm", "NT", "SWTI", "SWTII")]
#running OLS linear regression and output the diagrams for the linear regressions
#useCaseDataMelt <- gather(useCaseData, key=variable, value=value, NT, SWTI, SWTII)
...
#correlation between effort and NT
print("correlation between effort and NT")
cor1 = cor.test(data$Effort_Norm, data$NT)
print(cor1)
print(round(cor1$p.value,3))
print("linear regression of effort on NT")
m1 = lm(Effort_Norm~NT, data=data)
...
newx1 <- seq(-100, max(data$NT)+100, length.out=100)
...
#print(axis1)
m2 = lm(Effort_Norm~SWTI, data=data)
print("confidence interval for the parameters")
print(confint(m2))
#summary(m1.lm)$test[c("coefficients", "sigma", "tstat", "pvalues")]
...
cor3 = cor.test(data$Effort_Norm, data$SWTII)
print(cor3)
print(round(cor3$p.value,3))
print("linear regression of effort on SWTII")
#print(axis1)
m3 = lm(Effort_Norm~SWTII, data=data)
print("confidence interval for the parameters")
print(confint(m3))
#summary(m1.lm)$test[c("coefficients", "sigma", "tstat", "pvalues")]
...
newx3 <- seq(-100, max(data$SWTII)+100, length.out=100)
#print(newx3)
...
plot = xyplot(Effort_Norm~ value | variable, data=useCaseDataMelt,
		...
		ylab=list(label="Normalized Effort (PH)", fontsize=10),
		strip =strip.custom(factor.levels = c("NT","SWTI","SWTII")),
		#upper=data$upper,
		#lower=data$lower,
		scales=list(x=list(relation="free")),
		panel = function(x, y) {
...
for(i in 1:nfold){
	...
	print('nt testing set predication')
	nt.m = lm(Effort_Norm~NT, data=trainData)
	#nt.mre = apply(testData, 1, function(x))
	nt.predict = cbind(predicted=predict(nt.m, testData), actual=testData$Effort_Norm)
	print(nt.predict)
	nt.mre = apply(nt.predict, 1, function(x) abs(x[1] - x[2])/x[2])
	nt.mmre = mean(nt.mre)
	print(nt.mmre)
	#nt.preds = sapply(nt.mre, function(x) calculatePreds(x))
	nt.pred15 = length(nt.mre[nt.mre<=0.15])/length(nt.mre)
	nt.pred25 = length(nt.mre[nt.mre<=0.25])/length(nt.mre)
	nt.pred50 = length(nt.mre[nt.mre<=0.50])/length(nt.mre)
	print(c(nt.pred15, nt.pred25, nt.pred50))
	
	nt.pred <- 0
	for(j in 1:99){
		nt.pred <- c(nt.pred, length(nt.mre[nt.mre<=0.01*j])/length(nt.mre))
	}
...
print(foldResults);
cvResults <- c(
		mean(foldResults[, 'nt_mmre']),
		mean(foldResults[, 'nt_pred15']),
		mean(foldResults[, 'nt_pred25']),
		mean(foldResults[, 'nt_pred50']),
...
print('average improvement by nt')
print(colMeans(avgPreds[, "NT"] - avgPreds[,!colnames(avgPreds) %in% c("Pred")]))
...
#print(mean(avgPreds[, "SWTII"] - avgPreds[,"SWTI"]))
