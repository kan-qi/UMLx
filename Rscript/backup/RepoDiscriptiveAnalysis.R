...
############
projectDescriptiveData1 <- modelEvaluationData[c("NUM", "Type")]

#transactionalDataMelt <- gather(transactionalData, key=variable, value=value, EUCP_ALY, EXUCP_ALY, DUCP_ALY)
projectDescriptiveDataMelt1 <- melt(projectDescriptiveData1, id=c("NUM"))

print(projectDescriptiveDataMelt1)

png(filename=paste(outputDir,"project_type_distribution.png", sep="/"),
    units="in",
    width=4*2, 
    height=4*2, 
    pointsize=12,
    res=96)
projectHisto = histogram(~ value | variable,
                        main="Project Descriptive Statistics", 
                        ylab="Frequency",
                        xlab="1-Mobile App   2- Web App   3-Mobile Game  4-Mobile & Web App",
                        strip =strip.custom(factor.levels = c("Project Type")),
                        freq=TRUE,
                        bin.start=1,
                        bin.width=1,
                        breaks=20,
                        scales=list(x=list(relation="free")),
                        data=projectDescriptiveDataMelt1)
print(projectHisto)
############
