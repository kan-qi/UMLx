...
#a collection of transactions  =  system size
# for transaction, can classifed into different levels of complexity..

#1-10 => discretize => 1-3;4-7;>7 => classify => simple, average, complex => apply the weights => simple 3; average 5; complexity 10;

#1. study the distribution of the transactions with respect to properties.
#2. apply normal distribution
#3. try to understand the quartiles/percentiles -> corresponds to ranges of number of transactions.
#4. recalculate the size metrics
#5. evaluate using cross valdiation for MMRE and PRED.

# swtI, it uses the number of activities (called TL) SWT = sum of weighted transactions = system size
# swtII, it uses TL and TD (transaction degree) = the interface complexity (the number of public methods of the components)....
# swtIII, it uses TL, TD, DETs = number of data elements
