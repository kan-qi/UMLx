...
useCaseData <- data[c("Effort_Norm", "EUCP", "EXUCP", "DUCP")]
...
print(coeff1)
eucp.predict = cbind(predicted=predict(m1, data), actual=data$Effort_Norm)
...
print(coeff2)
exucp.predict = cbind(predicted=predict(m2, data), actual=data$Effort_Norm)
print("predicted values")
print(exucp.predict)
...
#print(axis1)
m3 = lm(Effort_Norm~DUCP, data=data)
#summary(m1.lm)$test[c("coefficients", "sigma", "tstat", "pvalues")]
...
print(coeff3)
ducp.predict = cbind(predicted=predict(m3, data), actual=data$Effort_Norm)
print("predicted values")
print(ducp.predict)
...
for(i in 1:nfold){
	...
	ducp.m = lm(Effort_Norm~DUCP, data=trainData)
	ducp.predict = cbind(predicted=predict(ducp.m, testData), actual=testData$Effort_Norm)
	print(ducp.predict)
