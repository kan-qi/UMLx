...
function highlight_diagram_element(idString, elementType, diagramType) {
  ...
  if (diagramType === "analysis_diagram") {
    const nodes = Array.prototype.slice.apply(
      svgDoc.getElementsByClassName("node")
    );
    ...
    const edges = Array.prototype.slice.apply(
      svgDoc.getElementsByClassName("edge")
    );
...
function highlight_activity_diagram(svgDoc, element) {
  ...
  if (element.getElementsByTagName("path")) {
    element
      ...
      .setAttribute(
        "style",
        "stroke:yellow;stroke-width:3px;stroke-opacity:0.5;"
      );
  }
}
...
function query_model_detail_func() {
  ...
  var modelId = url.substring(url.indexOf("=") + 1);
  console.log(modelId);
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      $('.model-info-content a[data-toggle="tab"]').on("shown.bs.tab", function(
        e
      ) {
      ...
      document.getElementById("model-nav").style.display = "block";

      $("#model-nav").append(
        "<li class='active'><a data-toggle='tab', href='#model-analytics'>Overview</a></li>" +
          "<li><a id='model-element', data-toggle='tab', href='#usecase-analysis'>Model Elements</a></li>" +
          "<li><a data-toggle='tab', href='#model-evaluation', id='model-evaluation-tab'>Model Info</a></li>" +
          "<li><a data-toggle='tab', href='#project-properties'>Project Info</a></li>" +
          "<li><a data-toggle='tab', id='modelArchive' href='#repo_archive', data-url='#{filePath}', data-type='archive', onclick='createDir(this);'>Archive</a></li>" +
          "<li><a data-toggle='tab', id='modelUploads' href='#loadsPath', data-url='#{loadsPath}', data-type='upload', onclick='createDir(this);'>Uploads</a></li>" +
          "<li class='dropdown'>" +
          "<a href='#' class='dropdown-toggle' data-toggle='dropdown' role='button' aria-haspopup='true' aria-expanded='false'> Dump Model" +
          "<span class='caret'></span></a>" +
          "<ul class='dropdown-menu'>" +
          "<li><a href='./#{response.modelInfo.AccessDir}/useCaseEvaluation.csv'>Evaluation For Use Cases</a></li>" +
          "<li><a href='./#{response.modelInfo.AccessDir}/domainModelEvaluation.csv'>Evaluation For Domain Models</a></li>" +
          "<li><a href='./#{response.modelInfo.AccessDir}/transactionAnalytics.csv'>Evaluation For Transactions</a></li>" +
          "<li><a href='./#{response.modelInfo.AccessDir}/entityAnalytics.csv'>Evaluation For Classes</a></li>" +
          "<li><a href='./#{response.modelInfo.AccessDir}/elementAnalytics.csv'>Evaluation For Elements</a></li>" +
          "</ul>" +
          "</li>" +
          "<li class='dropdown'>" +
          "<a href='#' class='dropdown-toggle' data-toggle='dropdown' role='button' aria-haspopup='true' aria-expanded='false'> Load" +
          "<span class='caret'></span></a>" +
          "<ul class='dropdown-menu'>" +
          "<li><a href='#'>Model Version</a></li>" +
          "</ul>" +
          "</li>" +
          "<li class='dropdown'>" +
          "<a href='#' class='dropdown-toggle' data-toggle='dropdown' role='button' aria-haspopup='true' aria-expanded='false'> Analysis" +
          "<span class='caret'></span></a>" +
          "<ul class='dropdown-menu'>" +
          "<li><a href='#'>Re-analyse Model</a></li>" +
          "<li><a href='#'>Delete Model</a></li>" +
          "</ul>" +
          "</li>"
      );

      // onClick='" + "query_model_usecase_func(\'" + modelId + "\'" + ");'" + "
      var modelElementTab = document.getElementById("model-element");
      modelElementTab.addEventListener("click", function() {
        query_model_usecase_func(modelId);
      });
...
function display_csv_data(url) {
  ...
  $("#overlay-frame .modal-body").html("");
  $("#overlay-frame .modal-body").html(
    "<img class='progress-bar-icon' src='img/progress-bar.gif'> Requesting data ..."
  );
  console.log(url);
  $.ajax({
    type: "GET",
    ...
    success: function(response) {
      ...
      $("#overlay-frame .modal-body").html("");
      var container = d3
        .select("#overlay-frame .modal-body")
        .append("table")
...
function query_estimation_models_func() {
  ...
  $("#model-query-progressing")
    .removeClass("hidden")
    .addClass("shown");
  $.ajax({
    type: "GET",
    url:
      "queryEstimationModel?estimator=" +
      estimator +
      "&model=" +
      model +
      "&repo_id=" +
...
function loginFormSubmit(e) {
  ...
  var usernameMissing =
    $("#login-form #username").val().length > 0 ? false : true;
...
function drawChartBySVG() {
  ...
  $("#repo-stats-chart").append(
    '<div id="model-selection-panel"> \
...
$(document).ready(function() {
  ...
  $(document).on(
    "click",
    "#use-case-evaluation-form-submit-button",
    use_case_evaluation_upload_fnc
  );
  $(document).on(
    "click",
    "#model-evaluation-form-submit-button",
    model_evaluation_upload_fnc
  );
  ...
  $(document).on(
    ...
    requestUseCaseDistributionCharts
  );
  ...
  $(document).on(
    "click",
    "#model-selector-box .dropdown-menu li a",
    function() {
      //        alert($(this).closest('.dropdown').find('.btn').text());
      $(this)
        .closest(".dropdown")
  ...
  $(document).on("click", "#EffortPH", function() {
    drawNewHighCharts("Effort", $("#EffortPH").text());
  });

  $(document).on("click", "#EffortDev", function() {
    drawNewHighCharts("useCasePMEffort", $("#EffortDev").text());
  });

  $(document).on("click", "#EffortPM", function() {
    drawNewHighCharts("useCaseDEVEffort", $("#EffortPM").text());
  });

  $(document).on("click", "#EffortSyn", function() {
    drawNewHighCharts("synthesizedEffort", $("#EffortSyn").text());
  });

  $(document).on("click", "#Business", function() {
    drawNewHighCharts("BusinessValue", $("#Business").text());
  });

  $(document).on("click", "#BVEffort", function() {
    drawNewHighCharts("BVEffortRatio", $("#BVEffort").text());
  });

  $(document).on("click", "#SizeMeasurement", function() {
    drawNewHighCharts("SizeMeasurement", $("#SizeMeasurement").text());
  });

  $(document).on("click", "#Duration", function() {
    drawNewHighCharts("Duration", $("#Duration").text());
  });

  $(document).on("click", "#Personnel", function() {
    drawNewHighCharts("Personnel", $("#Personnel").text());
  });

  $(document).on("click", "#Personnel_UI", function() {
    drawNewHighCharts("Personnel_UI", $("#Personnel_UI").text());
  });

  $(document).on("click", "#Personnel_FS", function() {
    drawNewHighCharts("Personnel_FS", $("#Personnel_FS").text());
  });

  $(document).on("click", "#Personnel_DB", function() {
    drawNewHighCharts("Personnel_DB", $("#Personnel_DB").text());
  });
...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  if (hash == "#overview") {
    ...
    document.getElementById("main_page").classList.remove("fade");
    ...
    document.getElementById("repo_trending").classList.remove("fade");
    ...
    document.getElementById("project_list").classList.remove("fade");
    ...
    var get = document.getElementById("modelArchive");
    console.log(get);
    walkRepoDir(get)
    // console.log("get: " + get);
    // walkRepoDir(get);
    ...
    document.getElementById("profile").classList.remove("fade");
  }
});

function openDialogueBox(repoId, type) {
  var formUseCase =
    '<form id="load-file-submit-form" onsubmit="load_file_upload_fnc(' +
    type +
    '); return false;"><div class="form-group"><input type="file" name="usecase-file" id="upload-file" class="form-control"></div><div> <p>The supported file type: .csv </p><input type="hidden" id="repo-id" name="repo-id" value="' +
    repoId +
    '"></div><div><input type="submit" class="btn btn-primary"></div></form>';
  var formModel =
    '<form id="load-file-submit-form" onsubmit="load_file_upload_fnc(' +
    type +
    '); return false;"><div class="form-group"><input type="file" name="model-file" id="upload-file" class="form-control"></div><div> <p>The supported file type: .csv </p><input type="hidden" id="repo-id" name="repo-id" value="' +
    repoId +
    '"></div><div><input type="submit" class="btn btn-primary"></div></form>';
  var formCOCOMO =
    '<form id="load-file-submit-form" onsubmit="load_file_upload_fnc(' +
    type +
    '); return false;"><div class="form-group"><input type="file" name="COCOMO-file" id="upload-file" class="form-control"></div><div> <p>The supported file type: .csv </p><input type="hidden" id="repo-id" name="repo-id" value="' +
    repoId +
    '"></div><div><input type="submit" class="btn btn-primary"></div></form>';
...
function openDialogueBoxForModelFileUpdate(repoId, modelId) {
  var form =
    '<form id="model-file-update-submit-form" onsubmit="model_file_update_fnc(); return false;"><div class="form-group"><input type="file" name="uml-file" id="uml-file" class="form-control"></div><div> <p>The supported file type: .xml </p><input type="hidden" id="repo-id" name="repo-id" value="' +
    repoId +
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      $("#main-panel").append(
        $(response)
          .children()
          .splice(1, $(response).children().length)
      );
    },
    error: function() {
      // $("#commentList").append($("#name").val() + "<br/>" +
...
function createTrendingLines() {
  $.ajax({
    ...
    success: function(response) {
      ...
      $("#project-card input").click(function() {
      ...
      $("#sloc-card input").click(function() {
        console.log("sloc clicked");
      ...
      $("#exucp-card input").click(function() {
      ...
      $("#usecase-card input").click(function() {
      ...
      $("#schedule-card input").click(function() {
      ...
      $("#ducp-card input").click(function() {
        console.log("ducp clicked");
      ...
      $("#transaction-card input").click(function() {
      ...
      $("#personnel-card input").click(function() {
        console.log("personnel clicked");
      ...
      $("#effort-card input").click(function() {
        console.log("effort clicked");
      ...
      $("#class-card input").click(function() {
      ...
      $("#eucp-card input").click(function() {
        console.log("eucp clicked");
      ...
      $("#estimate-card input").click(function() {
        console.log("estimate clicked");
        series_data.estimatedEffort.selected = !series_data.estimatedEffort
          .selected;
...
function createPieChart() {
  ...
  d3.csv(url, function(error, data) {
    ...
    var counter = [],
      transactionLable = [];
    data.forEach(function(d) {
      counter[d.transactional]
        ? counter[d.transactional]++
        : (counter[d.transactional] = 1);
...
function createHistogram(dataList, max) {
  var url = $("#model-analysis")[0]
    ? $("#model-analysis")[0].attributes.getNamedItem("data-expandedPathURL")
        .value
    : "";
  if (url) {
    d3.csv(url, function(error, data) {
      ...
      data.forEach(function(d) {
        var temp = newData[d.transactional]
          ? newData[d.transactional]
          : undefined;
        if (temp) {
          temp.list[+d.path_length]
            ? temp.list[+d.path_length]++
            : (temp.list[+d.path_length] = 1);
        }
      });
      var maxLength = 0;
      var dataList = [];
      for (var chartData in newData) {
        if (newData.hasOwnProperty(chartData)) {
          ...
          maxLength =
            maxLength < temp.list.length ? temp.list.length : maxLength;
...
function creatAvgHistograms() {
  var url = $("#model-analysis")[0]
    ? $("#model-analysis")[0].attributes.getNamedItem("data-pathAnalyticsURL")
        .value
    : "";
  if (url) {
    d3.csv(url, function(error, data) {
      ...
      data.forEach(function(d) {
        newData[0].list[+d.arch_diff]
          ? newData[0].list[+d.arch_diff]++
          : (newData[0].list[+d.arch_diff] = 1);
        newData[1].list[+d.avg_degree]
          ? newData[1].list[+d.avg_degree]++
          : (newData[1].list[+d.avg_degree] = 1);
        newData[2].list[+d.path_length]
          ? newData[2].list[+d.path_length]++
          : (newData[2].list[+d.path_length] = 1);
      });
      newData.forEach(function(chartData) {
        for (i = 0; i < chartData.list.length; i++) {
          chartData.list[i] = chartData.list[i] ? chartData.list[i] : 0;
        }
        createHistogramIndividually(
          chartData.id,
          chartData.list,
          chartData.xAxis,
          chartData.yAxis,
...
function createHistogramForPathNumber() {
  var url = $("#model-analysis")[0]
    ? $("#model-analysis")[0].attributes.getNamedItem(
        "data-usecaseAnalyticsURL"
      ).value
    : "";
  var list = [],
    id = "chart-11",
...
function createHistogramIndividually(
  id,
  data,
  xAxisName,
  yAxisName,
...
function editFunction(button) {
  ...
  document
    .getElementById("editNumber10")
    .attributes.removeNamedItem("disabled");
  document
    .getElementById("editNumber11")
    .attributes.removeNamedItem("disabled");
  document
    .getElementById("editNumber12")
    .attributes.removeNamedItem("disabled");
  document
    .getElementById("editNumber13")
    .attributes.removeNamedItem("disabled");
  document
    .getElementById("editNumber14")
    .attributes.removeNamedItem("disabled");
  document
    .getElementById("editNumber15")
    .attributes.removeNamedItem("disabled");
  document
    .getElementById("editNumber16")
    .attributes.removeNamedItem("disabled");
  document
    .getElementById("editNumber17")
    .attributes.removeNamedItem("disabled");
  document
    .getElementById("editNumber18")
    .attributes.removeNamedItem("disabled");
  document
    .getElementById("editNumber19")
    .attributes.removeNamedItem("disabled");
  document
    .getElementById("editNumber20")
    .attributes.removeNamedItem("disabled");
  document
    .getElementById("editNumber21")
    .attributes.removeNamedItem("disabled");
...
function submitEdit() {
  ...
  form_data.append(
    "includes_special_security_objectives",
    $("#editNumber11").val()
  );
  form_data.append(
    "provides_direct_access_for_third_parties",
    $("#editNumber12").val()
  );
  form_data.append(
    "special_user_training_facilities_are_required",
    $("#editNumber13").val()
  );
  form_data.append(
    "familiar_with_the_form_model_that_is_used",
    $("#editNumber14").val()
  );
...
function buildTable(data) {
  ...
  for (var i = 0; i < newKeys.length; i++) {
    ...
    dateData[data[i].url] =
      data[i].date.substring(0, 10) + " " + data[i].date.substring(11, 19);
  }
  ...
  if (data.length === 0) {
    ...
    if (tempUrl.length >= 3) {
      for (var i = 2; i < tempUrl.length - 1; i++) {
        storeUrl.push(
          tempUrl[i].substring(0, 8) + "..." + tempUrl[i].slice(-8)
        );
      }
      ...
      if (storeUrl.length == 0) {
        ...
        if (tempUrl[tempUrl.length - 1].length > 16) {
          displayUrl +=
            tempUrl[tempUrl.length - 1].substring(0, 8) +
            "..." +
            tempUrl[tempUrl.length - 1].slice(-8);
        } else {
    ...
    out +=
      "<p id='dirAddress' class='col-sm-offset-2 col-sm-10'>" +
      displayUrl +
      "</p></div>";
    out += "<table class='row table-striped' style='margin-left: 0'>";
    out +=
      "<tr><th style='text-align: left'>Name</th><th style='text-align: left'>File Type</th><th style='text-align: left'>Size</th><th style='text-align: left'>Creation Date</th><th style='text-align: left'>Submit</th></tr>";

    if (level >= 2) {
      out +=
        "<tr><td align='left'><button id='backButton' class='btn btn-default' data-url='" +
        parentUrl.substring(0, parentUrl.lastIndexOf("/")) +
        "' onclick='backDir(this)'>Back</button><td></tr>";
    }
    ...
    for (var i = 0; i < newKeys.length; i++) {
      if (type[newKeys[i]] === "Folder") {
        ...
        console.log(path);
        out +=
          "<tr><td><img style='width:30px; height:25px; margin-right: 10px' src='../img/folder-icon.png'><a href='#' id='div" +
          i +
          "' data-type=\"" +
          type[data[i].url] +
          '" data-url="' +
          path +
          "\" onclick='walkDir(this)'>" +
          newKeys[i] +
          "</a></td><td>folder</td>";
        out +=
          "<td>" +
          ...
          "</td>";
        var isChecked =
          projectSelectedArchives.indexOf(path) != -1 ? "checked" : "";
        out +=
          "<td>" +
    ...
    for (var i = 0; i < newKeys.length; i++) {
      if (type[newKeys[i]] === "File") {
        ...
        if (newKeys[i].endsWith(".csv")) {
          out +=
            "<tr><td><img style='width:30px; height:30px; margin-right: 10px' src='../img/csv.jpg'><a onclick='openFile(this)' data-path='" +
            path +
            "'>" +
            newKeys[i] +
            "</a></td><td>file</td>";
          ...
          out +=
            "<tr><td><img style='width:30px; height:30px; margin-right: 10px' src='../img/txt.jpg'><a onclick='openFile(this)' data-path='" +
            path +
            "'>" +
            newKeys[i] +
            "</a></td><td>file</td>";
          ...
          out +=
            "<tr><td><img style='width:30px; height:30px; margin-right: 10px' src='../img/svg.jpg'><a onclick='openFile(this)' data-path='" +
            path +
            "'>" +
            newKeys[i] +
            "</a></td><td>file</td>";
          ...
          out +=
            "<tr><td><img style='width:30px; height:30px; margin-right: 10px' src='../img/json.png'><a onclick='openFile(this)' data-path='" +
            path +
            "'>" +
            newKeys[i] +
            "</a></td><td>file</td>";
          ...
          out +=
            "<tr><td><img style='width:30px; height:30px; margin-right: 10px' src='../img/emptyfile.jpg'><a onclick='openFile(this)' data-path='" +
            path +
            "'>" +
            newKeys[i] +
            "</a></td><td>file</td>";
    ...
    out += "</table>";
    out +=
      "<div id='submitArchivesButton'><button class='btn btn-success' type='button' name='project' onclick='" +
      "refresh()" +
      "'>" +
      "Refresh" +
      "</button>";
    out +=
      "<button class='btn btn-default' type='button' name='project' onclick='" +
      "clearSelectedArchives(this)" +
      "'>" +
      "Clear" +
      "</button>";
    out +=
...
function buildTable2(data) {
  ...
  if (levels >= 2) {
    ...
    out +=
      "<tr><td align='left'><button id='backButton' class='btn btn-default' data-url='" +
      backUrl +
      "' onclick='walkRepoDir(this)'>Back</button></td></tr>";
  }
  ...
  for (var i = 0; i < newKeys.length; i++) {
    if (type[newKeys[i]] === "Folder") {
      ...
      out +=
        "<tr><td><img style='width:30px; height:25px; margin-right: 10px;' src='../img/folder-icon.png'><a href='#' class='file-link' id='div" +
        i +
        "' data-url=" +
        documentUrl +
        " onclick='walkRepoDir(this)'>" +
        newKeys[i] +
        "</a></td><td>folder</td>";
      out +=
        "<td>" +
  ...
  for (var i = 0; i < newKeys.length; i++) {
    if (type[newKeys[i]] === "File") {
      ...
      if (newKeys[i].endsWith(".csv")) {
        out +=
          "<tr><td><img style='width:30px; height:30px; margin-right: 10px;'  src='../img/csv.jpg'><a onclick='openFile(this)' class='file-link' data-path='" +
          path +
          "'>" +
          newKeys[i] +
          "</td><td>file</td>";
        ...
        out +=
          "<tr><td><img style='width:30px; height:30px; margin-right: 10px;' src='../img/txt.jpg'><a onclick='openFile(this)' class='file-link' data-path='" +
          path +
          "'>" +
          newKeys[i] +
          "</td><td>file</td>";
        ...
        out +=
          "<tr><td><img style='width:30px; height:30px; margin-right: 10px;' src='../img/svg.jpg'><a onclick='openFile(this)' class='file-link' data-path='" +
          path +
          "'>" +
          newKeys[i] +
          "</td><td>file</td>";
        ...
        out +=
          "<tr><td><img style='width:30px; height:30px; margin-right: 10px;' src='../img/json.png'><a onclick='openFile(this)' class='file-link' data-path='" +
          path +
          "'>" +
          newKeys[i] +
          "</td><td>file</td>";
        ...
        out +=
          "<tr><td><img style='width:30px; height:30px; margin-right: 10px;' src='../img/emptyfile.jpg'><a onclick='openFile(this)' class='file-link' data-path='" +
          path +
          "'>" +
          newKeys[i] +
          "</td><td>file</td>";
  ...
  out += "</table>";
  out +=
    "<div id='submitArchivesButton'><button class='btn btn-primary' style='background-color: #17A2B8' type='button' name='root' onclick='" +
    "submitSelectedArchives(this)" +
    "'>" +
    "Submit" +
    "</button>";
  out +=
    "<button class='btn btn-danger' type='button' name='root' onclick='" +
    "clearSelectedArchives(this)" +
    "'>" +
    "Clear" +
    "</button></div>";
...
function onCheckboxChange(checkboxElem) {
  const curSelectArchives =
    checkboxElem.name === "root"
      ? rootSelectedArchives
      : projectSelectedArchives;
...
function analyseSLOC(e) {
  ...
  // }
  const curSelectArchives =
    e.name === "root" ? rootSelectedArchives : projectSelectedArchives;
  $.ajax({
    type: "POST",
    url: "analyseSLOC",
    data: {
      archives: JSON.stringify(curSelectArchives),
      workingDir: fileFolder
    },
    success: function(response) {
      console.log(response);
    },
...
function analyseProjectSrc(e) {
  ...
  $("#file-list-panel-body").html(
    "<div><p>" + path + '</p><div id="displayArchive"></div></div>'
  );
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      for (var i in projectList) {
        ...
        htmlContent +=
          '<div><a onclick="analyseProjectSrc(this)" data-url="' +
          project.path +
          '">' +
          project.tag +
          "</a></div>";
...
const publicVapidKey =
  "BM2EKwsY9E_5r5ewHVlZ1hSwpSfRpvqQm0DPT3C60WQ3md98O0_Tb7c56yFfzFlFyaKqNVfYe1Vv2sul6m4Myt0";
startEvalPush();
...
function estimationOptionChange(selectorId) {
  ...
  setEstimationCookie(
    EstimationCookieName,
    JSON.stringify(EstimationCookieObject),
    30
  );
}
