...
import presto.android.Debug1;
import presto.android.Debug4;
import presto.android.Debug2;
import presto.android.Debug5;

/**
 * @author Kan Qi
 *
 */
public class CodeAnalysis3 {
	private static CodeAnalysis3 theInstance;

	private CodeAnalysis3() {
	}

	public static synchronized CodeAnalysis3 v() {
		if (theInstance == null) {
			theInstance = new CodeAnalysis3();
		}
		return theInstance;
	}
	...
	public String constructAccessGraph(List<ClassUnit> classUnits, List<CompositeClassUnit> compositeClassUnits, Map<String, ClassUnit> classUnitByName, Map<String, ClassUnit> classUnitByUUID, Map<String, CompositeClassUnit> compositeClassUnitByUUID, Map<String, String> classUnitToCompositeClassDic) {
		...
		for (ClassUnit classUnit : classUnits) {
			...
			for (MethodUnit methodUnit : methodUnits) {
				...
				presto.android.Debug2.v().printf("method:%s", methodUnit.name);
				...
				presto.android.Debug2.v().printf("active body:%s", methodUnit.name);
...
//				        Debug2.v().printf("field:%s\n", fieldRef.toString());
	...
	public void run() {
		presto.android.Logger.stat("#Classes: " + Scene.v().getClasses().size() +
				", #AppClasses: " + Scene.v().getApplicationClasses().size());
		presto.android.Logger.trace("TIMECOST", "Start at " + System.currentTimeMillis());
