...
//  private ExecutorService executor = Executors.newFixedThreadPool(1);
  ...
  private static CFGWriter1 theInstance;

  // time analysis begins
  private long startTime;

  private CFGWriter1() throws Exception {
    debugFileName = Configs.outputDir+"/soot-cfg-1.json";
...
//    out = new PrintWriter(new BufferedWriter(new FileWriter(debugFileName)));
  ...
  public static synchronized CFGWriter1 v() {
    if (theInstance == null) {
      try {
        theInstance = new CFGWriter1();
      } catch (Exception e) {
