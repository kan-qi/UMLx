...
//  private ExecutorService executor = Executors.newFixedThreadPool(1);
  ...
  private static CFGWriter2 theInstance;

  // time analysis begins
  private long startTime;

  private CFGWriter2() throws Exception {
    debugFileName = Configs.outputDir+"/soot-cfg-2.json";
...
//    out = new PrintWriter(new BufferedWriter(new FileWriter(debugFileName)));
  ...
  public static synchronized CFGWriter2 v() {
    if (theInstance == null) {
      try {
        theInstance = new CFGWriter2();
      } catch (Exception e) {
