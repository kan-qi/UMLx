...
//  private ExecutorService executor = Executors.newFixedThreadPool(1);
  ...
  private static ClassWriter theInstance;

  // time analysis begins
  private long startTime;

  private ClassWriter() throws Exception {
    debugFileName = "./soot-classes.txt";
...
//    out = new PrintWriter(new BufferedWriter(new FileWriter(debugFileName)));
  ...
  public static synchronized ClassWriter v() {
    if (theInstance == null) {
      try {
        theInstance = new ClassWriter();
      } catch (Exception e) {
