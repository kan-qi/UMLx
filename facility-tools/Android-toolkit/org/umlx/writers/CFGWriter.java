...
//  private ExecutorService executor = Executors.newFixedThreadPool(1);
  ...
  private static CFGWriter theInstance;

  // time analysis begins
  private long startTime;

  private CFGWriter() throws Exception {
    debugFileName = Configs.outputDir+"/soot-cfg.txt";
...
//    out = new PrintWriter(new BufferedWriter(new FileWriter(debugFileName)));
  ...
  public static synchronized CFGWriter v() {
    if (theInstance == null) {
      try {
        theInstance = new CFGWriter();
      } catch (Exception e) {
