...
public class GatorHandlersWriter {
  // codes
...
//  private ExecutorService executor = Executors.newFixedThreadPool(1);
  ...
  private static GatorHandlersWriter theInstance;

  // time analysis begins
  private long startTime;

  private GatorHandlersWriter() throws Exception {
    debugFileName = Configs.outputDir+"/gator-handlers.txt";
...
//    out = new PrintWriter(new BufferedWriter(new FileWriter(debugFileName)));
  ...
  public static synchronized GatorHandlersWriter v() {
    if (theInstance == null) {
      try {
        theInstance = new GatorHandlersWriter();
      } catch (Exception e) {
