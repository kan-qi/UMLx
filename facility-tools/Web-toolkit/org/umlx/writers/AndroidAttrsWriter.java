...
public class AndroidAttrsWriter {
  // codes
...
//  private ExecutorService executor = Executors.newFixedThreadPool(1);
  ...
  private static AndroidAttrsWriter theInstance;

  // time analysis begins
  private long startTime;

  private AndroidAttrsWriter() throws Exception {
    debugFileName = Configs.outputDir+"/android-attrs.json";
...
//    out = new PrintWriter(new BufferedWriter(new FileWriter(debugFileName)));
  ...
  public static synchronized AndroidAttrsWriter v() {
    if (theInstance == null) {
      try {
        theInstance = new AndroidAttrsWriter();
      } catch (Exception e) {
...
//Put this in another thread if calling it frequently hurts performance.
 public void print(String outputS) {
   out.print(outputS);
 }
