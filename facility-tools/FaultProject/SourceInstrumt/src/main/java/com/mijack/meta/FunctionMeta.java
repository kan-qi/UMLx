...
//            String elementName = Utils.elementToString(nameElement);
//            if(!elementName.equals("")) {
//            logStmt.append(",").append(elementName);
//            }
//            i = i + 1;
//        }
        logStmt.append(");");
        ...
        //need to detect if the first statement is super or this
        boolean hasSuper = false;
        Iterator nodeItr = blockElement.nodeIterator();
        while (nodeItr.hasNext()) {
            Object o = nodeItr.next();
            if(!(o instanceof Element)) {
            	continue;
            }
            Element e = (Element)o;
            if(!e.getName().equals("expr_stmt")) {
            	continue;
            }

            List<String> superFuncs = findSuperThisCalls(e);
            
//            System.out.println("func node");
//            System.out.println(e.toString());
            if(superFuncs.size() > 0) {
            	hasSuper = true;
            }
        }
...
//                count==0
    ...
    private List<String> findSuperThisCalls(Element element) {
    	List<String> funcCalls = new ArrayList<String>();
//    	System.out.println("find function calls");
    	
//    	String funcCall = "";
    	if(element.getName().equals("call")) {
//    		System.out.println("found call");
    		Node funcPart = element.node(0);
    		if(funcPart instanceof Element && funcPart.getName().equals("name")&& (funcPart.getText().equals("super")||funcPart.getText().equals("this"))) {
    			System.out.println("found super or this function");
    			System.out.println(funcPart.getText());
    			funcCalls.add(funcPart.getText());
//        		Node argsPart = element.node(1);
//        		if(argsPart instanceof Element && argsPart.getName().equals("argument_list")) {
//        			Iterator argItr = ((Element)argsPart).nodeIterator();
//            		while(argItr.hasNext()) {
//            			Object arg = argItr.next();
//            			if(arg instanceof Element && ((Element)arg).getName().equals("argument")) {
//            				System.out.println(((Element)((Element)arg).node(0)).node(0));
//            			}
//            		}
////        			System.out.println().node(0));
//            		return funcCalls.toArray(new String[]{});
//        		}
    		}
    	}
...
//    		if(element.nodeCount() > 1) {
    			...
    			if(o instanceof Element) {
    			List<String> subFuncCalls = findSuperThisCalls((Element)o);
    			funcCalls.addAll(subFuncCalls);
    			}
//    		}
    	}
    ...
    private Element getLastElement(Element parent) {
        ...
        Iterator<Node> nodeIterable = parent.nodeIterator();
        while (nodeIterable.hasNext()) {
            Node next = nodeIterable.next();
            if (next instanceof Element) {
                lastNode = (Element)next;
            }
        }
        return lastNode;
    }
    ...
    public void insertExitLog() {
        ...
        boolean isLastLineThrow = false;
        
        if(blockElement.nodeCount() > 0) {
        	Element lastElement = getLastElement(blockElement);
//    		System.out.println(((Element)lastNode).getName());
        	if(lastElement != null && lastElement instanceof Element) {
        		System.out.println("last element");
        		System.out.println(lastElement.toString());
        		isLastLineThrow = lastElement.getName().equals("throw");
        		if(isLastLineThrow) {
        			System.out.println("is throw");
        		}
        	}
        }
        ...
        if(isLastLineThrow) {
            ...
            blockElement.remove(lastNode);
            blockElement.remove(secondLastNode);
            blockElement.add(logExit);
            blockElement.add(secondLastNode);
