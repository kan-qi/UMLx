...
```
## Adjusted r²
Exhaustive search up to 9 variables for model with highest adjusted r².
```{r echo=FALSE, error=FALSE, message=FALSE, warning=FALSE}
library(leaps)
models <- regsubsets(x = as.matrix(X_data), y = as.vector(Y_data), nbest = 1, method = "exhaustive")
models.summary <- summary(models)
t(models.summary$which[9,])
```
## AIC/BIC
Performed stepwise with forward and backward selection starting from the 9-variable model obtained above in the adjusted r² analysis.
```{r}
start <- lm(Y_data ~ Average_Depth_Inheritance_Tree + Number_Of_Inheritance_Relationships + Number_Of_Children + Depth_Inheritance_Tree + FUNC_NA + INT + UEXUCW + TCF + DUCP, data = X_data)
null <- lm(Y_data ~ 1, data = X_data)
full <- lm(Y_data ~ ., data = X_data)
```
AIC
```{r}
AIC_model <- step(start, scope = list(upper = full), data = X_data, direction = "both")
```
BIC
```{r}
BIC_model <- step(start, scope = list(upper = full), data = X_data, direction = "both", k = log(ncol(X_data)))
```
## Bayes Factor
```{r}
library(BayesFactor)
BF <- regressionBF(Y_data ~ ., data = X_data)
```
