...
modelData <- read.csv("modelEvaluations-1-3.csv")
#modelData <- read.csv("modelEvaluations-9-29-1.csv")
...
transactionFileList1 <- subset(modelData1, select=c("transaction_file"))
rownames(transactionFileList1) <- modelData1$Project

#EUCP
weights <- c(1)
names(weights) <- c("L1")
cutpoints <- NULL
results <- prodByLinearRegression(effort1, transactionFileList1, cutpoints, weights)
print(results)
...
TN Search Results
```{r swti part I} 
TNresults <- performSearch(6, effort, combined, transactionFiles, c())
...
```{r swti part I} 
SWTIresults <- performSearch(6, effort, combined, transactionFiles, c("TL"))
# Plot classification results
...
```{r swti part II}
SWTIresults_II <- performSearch(6, effort, combined, transactionFiles, c("TD"))
# Plot classification results
...
```{r swti part III}
SWTIresults_III <- performSearch(6, effort, combined, transactionFiles, c("DETs"))
  # Plot classification results
...
#draw example of posterior estimates
  ...
  colnames(marginalData) <- c(paste("l", 1:7, sep=""), "Effort Adj.")
    ...
    means <- apply(marginalData, 2, mean)
    sd <- apply(marginalData, 2, sd)
    labels <- paste(colnames(marginalData), ": µ~", round(means,2),"; ??~", round(sd,2),sep="")
    print(labels)
...
meltMergedMMREPREDII$Model <- rep("EXUCP", length(meltMergedMMREPREDII)) 
meltMergedMMREPREDIII$Model <- rep("DUCP", length(meltMergedMMREPREDIII)) 
meltMergedMMREPRED <- rbind(meltMergedMMREPREDII, meltMergedMMREPREDIII)
#meltMergedMMREPRED$Rank <- unlist(with(meltMergedMMREPRED,tapply(Value,list(Model),rank)))

meltMergedMMREPRED$Value <- apply(meltMergedMMREPRED, 1, function(x){
  print(x)
  if(x['Method'] == "PRED"){
    -as.numeric(x['Value'])
    }
...
require(dplyr)
meltMergedMMREPRED <- meltMergedMMREPRED %>% 
...
sumRank <- as.data.frame(t(tapply(meltMergedMMREPRED$Rank, meltMergedMMREPRED[, c("Model","Bins")], sum)))
...
meltMergedMMREPRED <- merge(sumRank, meltMergedMMREPRED, by=c("Bins", "Model"), all=TRUE)
          
print.data.frame(meltMergedMMREPRED)

meltMergedMMREPREDCols <- unique(meltMergedMMREPRED[, c("Bins","Model","SumRank","R2")])
meltMergedMMREPREDCols <- meltMergedMMREPREDCols %>% 
          arrange(Model)
#meltMergedMMREPREDCols$Label <- paste("Bins:", meltMergedMMREPREDCols$Bins, "\n", "Rank:", meltMergedMMREPREDCols$SumRank, "\n", "R2:", round(meltMergedMMREPREDCols$R2, 2), sep = "")

#meltMergedMMREPREDCols$Label <- paste(meltMergedMMREPREDCols$Bins, "\n", meltMergedMMREPREDCols$SumRank, "\n", substr(meltMergedMMREPREDCols$R2,3,nchar(as.character(meltMergedMMREPREDCols$R2))), sep = "")

meltMergedMMREPREDCols$Label <- paste(meltMergedMMREPREDCols$Bins, "\n", meltMergedMMREPREDCols$SumRank, "\n", substr(meltMergedMMREPREDCols$R2,4,7), sep = "")

#ggplot(data = meltMergedMMREPRED, aes(x = Bins, y = Value)) + geom_point(shape = 1) + geom_line() + facet_wrap(Model~Method, scales="free_y", ncol=2) + theme_bw()
ggplot(data = meltMergedMMREPRED, aes(x = Bins, y = Value)) + geom_point(shape = 1) +geom_text(aes(label=Rank),hjust=0, vjust=0) + geom_line() +facet_grid(vars(Method), vars(Model), scales="free_y") +scale_x_discrete(aes(breaks = Bins), labels=meltMergedMMREPREDCols$Label[7:12],limits=1:6)+theme_bw()+xlab("Bins")+ylab("Accuracy Measurements")
#labels=meltMergedMMREPREDCols$Label,limits=1:6)+theme_bw()+xlab("Bins")+ylab("Accuracy Measurements")
#ggplot(data = meltMergedMMREPRED, aes(x = Bins, y = Value)) + geom_point(shape = 1) +geom_text(aes(label=Rank),hjust=0.5, vjust=2) + geom_line() + facet_grid(vars(Method), vars(Model), scales="free_y") + theme_bw()
...
otherSizeMetricsData=modelData[c("Effort", "COCOMO_Estimate", "Priori_COCOMO_Estimate", "UCP", "IFPUG", "MKII", "COSMIC")]
otherSizeMetricsData <- na.omit(otherSizeMetricsData)

comparisonRet <- compareBetweenSizeMetrics(TNModelData, SWTIIModelData, SWTIIIModelData, otherSizeMetricsData)
#for testing
...
avgPreds_DUCP_UCP_FP <- avgPreds[, c("EUCP", "EXUCP", "DUCP", "IFPUG", "MKII", "COSMIC", "Pred")]
colnames(avgPreds_DUCP_UCP_FP) <- c("EUCP", "EXUCP", "DUCP", "IFPUG", "MKII", "COSMIC", "Pred")
#print(avgPreds)
...
TNModelParameters[["effortAdj"]] = empiricalStats(TNresults[[TNModelSelector]][["model"]][["normFactor"]])
TNModelParameters[["sigma"]] = empiricalStats(TNresults[[TNModelSelector]][["model"]][["sd"]])
...
for(i in levels){
  TNModelParameters[["levels"]][[i]] = empiricalStats(TNresults[[TNModelSelector]][["model"]][[i]])
}
...
SWTIIModelParameters[["effortAdj"]] = empiricalStats(SWTIIresults[[SWTIIModelSelector]][["model"]][["normFactor"]])
SWTIIModelParameters[["sigma"]] = empiricalStats(SWTIIresults[[SWTIIModelSelector]][["model"]][["sd"]])
...
for(i in levels){
  SWTIIModelParameters[["levels"]][[i]] = empiricalStats(SWTIIresults[[SWTIIModelSelector]][["model"]][[i]])
}
