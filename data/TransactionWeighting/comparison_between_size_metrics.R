compareBetweenSizeMetrics <- function(data){
...
for(i in 1:nfold){
	...
	eucp.m = lm(Effort~EUCP, data=trainData)
	#eucp.mre = apply(testData, 1, function(x))
	eucp.predict = cbind(predicted=predict(eucp.m, testData), actual=testData$Effort)
	print(eucp.predict)
	...
	exucp.m = lm(Effort~EXUCP, data=trainData)
	exucp.predict = cbind(predicted=predict(exucp.m, testData), actual=testData$Effort)
	print(exucp.predict)
	...
	ducp.m = lm(Effort~DUCP, data=trainData)
	ducp.predict = cbind(predicted=predict(ducp.m, testData), actual=testData$Effort)
	print(ducp.predict)
