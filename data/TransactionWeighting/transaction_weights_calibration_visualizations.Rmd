...
source("transaction_weights_calibration2.R")
library(tidyverse)
...
```{r swti part II}
SWTIresults <- performSearch(6, "./Transaction Data", effort, c("TD"))
# Plot classification results
for (i in 1:length(SWTIresults)) {
  result <- SWTIresults[[i]]
  df <- as.data.frame(t(subset(result$data, select = -c(Effort))))
...
```{r swti part III}
SWTIresults <- performSearch(3, "./Transaction Data", effort, c("DETs"))
# Plot classification results
for (i in 1:length(SWTIresults)) {
  result <- SWTIresults[[i]]
  df <- as.data.frame(t(subset(result$data, select = -c(Effort))))
...
SWTIIIresults <- performSearch(3, "./Transaction Data", effort, c("TL", "TD", "DETs"))
# Plot classification results
