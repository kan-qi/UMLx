...
for (i in 1:length(SWTIresults)) {
  ...
  print(categorizedTransactions)
   ...
   g <- ggplot(data = as.data.frame(categorizedTransactions), aes(x = Bins, y = Counts)) + 
    geom_bar(stat = "identity") +
...
#draw example of posterior estimates
  marginalData <- SWTIIIresults[[4]]$model[, !names(SWTIIIresults[[4]]$model) %in% c("(Intercept)", "sigma")]
