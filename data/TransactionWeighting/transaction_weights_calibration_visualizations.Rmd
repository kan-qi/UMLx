...
ggplot(data = MMREdata, aes(x = NumBins, y = MMRE)) + geom_point(shape = 1) + geom_line()
ggplot(data = PREDdata, aes(x = NumBins, y = PRED)) + geom_point(shape = 1) + geom_line() + ylab("PRED(0.25)")
...
for (i in 1:length(SWTIresults)) {
  ...
  if (is.vector(marginalData)) {
    marginalData <- data.frame(TL1 = marginalData)
  }
  for (col in names(marginalData)) {
    ...
    xlab(col) + ggtitle(paste(col, i, "bins")) +
...
for (i in 1:length(SWTIIresults)) {
  ...
  if (is.vector(marginalData)) {
    marginalData <- data.frame(TL1TD1 = marginalData)
  }
  for (col in names(marginalData)) {
    mean <- mean(marginalData[, col])
    sd <- sd(marginalData[, col])
