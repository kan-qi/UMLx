...
proposalfunction <- function(B, normFactor, sd, test_factor){
  #sd <- 10
  ...
  if(length(B)>1){
    for (i in 1:length(B)) {
      sigma <- c(sigma, test_factor* (abs(B[i] - B[i - 1]))+0.1)
    }
  }
  #print(sigma)
...
run_metropolis_MCMC <- function(regressionData, N, priorB, varianceMatrix, normFactor,var,test_factor){
  #priorB <- B
  ...
  #acceptance <- c()
  for (i in 1:N){
    proposal = proposalfunction(chain[i,names(priorB)], chain[i,"normFactor"], chain[i, "sd"],test_factor)
    #proposal = sample
    `%ni%` <- Negate(`%in%`)
...
bayesfit3<-function(regressionData, N, B, varianceMatrix, normFactor, var,test_factor){
  #N <-1000
  #B <- means
  ...
  #startvalue = c(4,0,10)
  chain = run_metropolis_MCMC(regressionData, N, B, varianceMatrix, normFactor, var,test_factor)
...
performSearch <- function(n, effortData, transactionFiles, parameters = c("TL", "TD", "DETs"), k = 5,test_factor) {
  ...
  for (i in seq(1,n)) {
    ...
    bayesianModel <- bayesfit3(regressionData[rownames(regressionData) != "Aggregate", ], 10000, means, covar, normFactor['mean'], normFactor['var'],test_factor)
