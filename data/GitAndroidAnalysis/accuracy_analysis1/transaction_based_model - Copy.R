...
performSearch <- function(n, dataset, parameters = c("TL", "TD", "DETs"), k = 5) {
  ...
  for (i in seq(1,n)) {
    ...
    bm_validationResults <- crossValidate(regressionData, k, function(trainData){
    ...
    transactionSum1 <- as.matrix(regressionData[, !(colnames(regressionData) %in% c("Effort"))])%*% nominalWeights
    ...
    #print(priorModel)
    validationResults2 <- crossValidate(transactionregressionData, k,function(trainData) lm(Effort ~ . - 1, as.data.frame(trainData)), function(lm.fit, testData) predict(lm.fit, newData = testData))
