...
$.ajax({
    ...
    success: function(response)
    {
        ...
        for (var i=0;i<response.length;i++)
        {
            ...
            //console.log($('#r').html());
            $('#repo_table').append("<tr align='center'><td>"+response[i]["column name"] +"</td><td>" + response[i].statistics.mean + "</td><td>" +  response[i].statistics.variance + "</td><td>" +  response[i].statistics.first_quartile + "</td><td>" +  response[i].statistics.median + "</td><td>" + response[i].statistics.third_quartile + "</td><td>" + response[i].statistics.kurtosis + "</td><td><input class='table-button' type='image' src='img/display_btn.png' onclick = 'showRepoModal(\"" + img_link3 +  "\");return false;' value=''> <input class='table-button' type='image' src='img/dist_btn.png' onclick='showDistributionChart(\"" + repo_url1 + "\",\"" + response[i]["column name"] + "\",\"" + '#myModal' + "\",\"" + '#model-body' +"\");return false;' value=''> <input type='checkbox' name='checkbox' value=\"" + response[i]["column name"] + "\"></td></tr>");
        }
    ...
    $.ajax({
        ...
        success: function(response)
        {
            ...
            for (var i=0;i<response.length;i++)
            {
                ...
                $('#usecase_table').append("<tr align='center'><td>"+response[i]["column name"] +"</td><td>" + response[i].statistics.mean + "</td><td>" +  response[i].statistics.variance + "</td><td>" +  response[i].statistics.first_quartile + "</td><td>" +  response[i].statistics.median + "</td><td>" + response[i].statistics.third_quartile + "</td><td>" + response[i].statistics.kurtosis + "</td><td><input type='image' class='table-button' src='img/display_btn.png' onclick = 'showUseCaseUrlModal(\"" + img_link4 +  "\");return false;' value=''> <input type='image' class='table-button' onclick='showDistributionChart(\"" + usecase_url3 + "\",\"" + response[i]["column name"] + "\",\"" + '#myModal3' + "\",\"" + '#model3-body' + "\");return false;' value='' src='img/dist_btn.png'> <input type='checkbox' name='checkbox' value=\"" + response[i]["column name"] + "\"></td></tr>");
            }
            $('.repo-metrics').append( "</table></form>");
        },
        error:function(error){
