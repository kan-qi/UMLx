...
# saves benchmarkResults for list of models under folder 'models' for the dataset <dataset_name>
# directory name is relative to currnt folder
saveBenchmarkResults <- function(benchmarkResults, dataset_name, directory){
  model_names = benchmarkResults$model_names
  accuracy_metrics = benchmarkResults$accuracy_metrics
  goodness_fit_metrics = benchmarkResults$goodness_fit_metrics
  ...
  for(model in model_names){
    jsonOUT = "{"
    # name of dataset used in model fitting
    jsonOUT = paste(jsonOUT,sprintf("\"dataset_name\":\"%s\"", dataset_name),sep = "")
    jsonOUT = paste(jsonOUT,",",sep = "")
    
    # model name
    jsonOUT = paste(jsonOUT,sprintf("\"model_name\":\"%s\"", model),sep = "")
    jsonOUT = paste(jsonOUT,",",sep = "")
    
    # save path
    jsonOUT = paste(jsonOUT,sprintf("\"model_path\":\".models/%s.Rdata\",", model), sep = "")
    
    # date time
    jsonOUT = paste(jsonOUT,sprintf("\"Date_created\":\"%s\",", as.character(Sys.time())), sep = "")
    # cvResults
    jsonOUT = paste(jsonOUT, "\"cvResults\":{", sep = "")
    # accuracy metrics
    jsonOUT = paste(jsonOUT,sprintf("\"accuracy_metrics\":"), sep = "")
    jsonOUT = paste(jsonOUT,"{",sep = "")
    for (metric in accuracy_metrics){
      metric = paste(model,metric,sep = "_")
      metric_val = benchmarkResults$cvResults$accuracyResults[metric]
      jsonOUT = paste(jsonOUT, sprintf("\"%s\":%f,", metric, metric_val), sep = "")
    }
    # remove ',' from last entry of accuracy metrics
    jsonOUT = substring(jsonOUT,1,nchar(jsonOUT)-1)
    jsonOUT = paste(jsonOUT,"},", sep = "")
    
    # goodness of fit metrics
    jsonOUT = paste(jsonOUT,sprintf("\"goodness_fit_metrics\":"), sep = "")
    jsonOUT = paste(jsonOUT,"{", sep = "")
    for(metric in goodness_fit_metrics){
      metric_val = benchmarkResults$fitResults[[model]][[metric]]
      jsonOUT = paste(jsonOUT, sprintf("\"%s\":%f,", metric, metric_val), sep = "")
    }
    # remove ',' from last entry of goodness fit metrics
    jsonOUT = substring(jsonOUT,1,nchar(jsonOUT)-1)
    jsonOUT = paste(jsonOUT, "}", sep = "")
    jsonOUT = paste(jsonOUT, "},", sep = "")
    
    # bsEstimations
    jsonOUT = paste(jsonOUT, "\"bsEstimations\":{", sep = "")
    bsEstimations = benchmarkResults$bsResults$bsEstimations
    for (metric in accuracy_metrics){
      metric = paste(model,metric,sep = "_")
      metric_val = bsEstimations[[metric]]
      jsonOUT = paste(jsonOUT, sprintf("\"%s\":[%f,%f,%f],", metric, metric_val[1], metric_val[2], metric_val[3]), sep = "")
    }
    # remove ',' from last entry of bsEstimations
    jsonOUT = substring(jsonOUT,1,nchar(jsonOUT)-1)
    jsonOUT = paste(jsonOUT, "}", sep = "")
    jsonOUT = paste(jsonOUT, "}", sep = "")
    
    filename = paste(directory,sprintf("/%s.json",model),sep='')
    write(jsonOUT,filename)
  }
}
