...
prior <- function(sample, B, varianceMatrix, normFactor, var){
  ...
  prior['normFactor'] = dunif(sample["normFactor"], min=0, max=2000, log = T)
  #print(prior)
  #print(sample)
...
posterior <- function(sample, B, varianceMatrix, normFactor, var, x, y){
  ...
  #y = regressionData[,c("Effort")]
  priorb = prior(sample, B, varianceMatrix, normFactor, var)
  likeh = likelihood(sample[names(B)], sample['normFactor'], sample['sd'], x, y)
  return  (likeh + priorb)
}
...
bayesfit<-function(regressionData, N){
  ...
  chain = run_metropolis_MCMC(regressionData, N, means, covar, normFactor['mean'],  normFactor['var'])
...
performSearch <- function(n, dataset, parameters = c("TL", "TD", "DETs"), k = 5) {
  ...
  for (i in seq(1,n)) {
    ...
    paramVals <- bayesfit(regressionData, 10000)
    bayesianModel = list()
    ...
    bm_validationResults <- crossValidate(regressionData, k, function(trainData) bayesfit(trainData, 10000), predict.blm)
    ...
    prior_validationResults <- crossValidate(transactionregressionData, k, priorFit, function(priorModel, testData) predict(priorModel, newData = testData))
...
predict.swt <- function(trainedModel, testData){
  ...
  predicted <- predict.blm(t(as.matrix(trainedModel$paramVals)), newdata = regressionData)
  predicted
}
...
m_fit.tm1 <- function(swtiii,dataset){
  ...
  paramVals <- bayesfit(regressionData, 100000)
  bayesianModel = list()
