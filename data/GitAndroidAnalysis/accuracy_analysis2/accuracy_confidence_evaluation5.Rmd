...
modelData <- selectData("dsets/combined_data_set_4_rufus.csv")
...
transaction_models <- trainsaction_based_model1(modelData)
...
model_names <- model_names[!model_names %in% c("tm2", "tm3")]
...
goodnessRankResults = data.frame(model_labels = names(fitResults))
...
write.csv(goodnessRankResults, "goodness_rank_results_8_14.csv")
```

```{r evaluate the out-of-sample accuracy of the models}
#evaluate cross-validation

#plot for the cross validation results for pred(.01)- pred(0.50)
...
for(i in 1:length(accuracy_metrics)){
  ...
  p[[i]] <- ggplot(selectedData, aes(x=model_names, y=rank, fill=model_names)) +
...
names(selectedData) <- c("rank");
p_cvAllRank <- ggplot(selectedData, aes(x=model_names, y=rank, fill=model_names)) +
...
filteredNonOverlappingPairs = nonOverlappingPairs[which((nonOverlappingPairs$Model1 %in% c("tm3", "tm2", "tm1") | nonOverlappingPairs$Model2 %in% c("tm3", "tm2", "tm1"))) & nonOverlappingPairs$Metric %in% c("mmre", "mdmre", "pred25"),]
