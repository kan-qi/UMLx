...
dataset <- read.csv("dsets/combined_data_set_8_28.csv")
...
write.csv(outputData, paste(outputDir, "output_data.csv", sep="/"))
...
model_texts <- c("SWTIII","SWTI","SWTII", "UCP", "IFPUG", "COSMIC", "SLOC", "LOG_SLOC")
...
print(avg_pred_plot)
ggsave(paste(outputDir, "avg_pred_plot.png", sep="/"), width=5, height=4)
...
sig_bs_f_1 = sig_bs[which(sig_bs$bonferroni_p_value < 0.05),]
...
#order models
sig_bs_f_1 <- as.data.frame(t(apply(sig_bs_f_1, 1, function(x) {
  #print(x)
    if(x['direction'] == "-"){
      model_2 = x['model2']
      x['model2'] = x['model1']
...
sig_bs_f_1$model1 = model_mapping$model_texts[match(sig_bs_f_1$model1, model_mapping$model_names)]
sig_bs_f_1$model2 = model_mapping$model_texts[match(sig_bs_f_1$model2, model_mapping$model_names)]
...
sig_bs_tm = sig_bs_f[which(sig_bs_f$model1 %in% c("tm1", "tm2", "tm3") | sig_bs_f$model2 %in% c("tm1", "tm2", "tm3")),]
...
`%ni%` <- Negate(`%in%`)
filtered_sig_bs_f = sig_bs_f[which(sig_bs_f$index %ni% filteredNonOverlappingPairsIndices), ]

write.csv(filtered_sig_bs_f,paste(outputDir, 'filtered_sig_bs_f.csv', sep="/"))
