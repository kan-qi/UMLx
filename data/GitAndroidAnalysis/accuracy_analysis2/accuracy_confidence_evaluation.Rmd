...
#fitted_model <- fit(modelData, "lasso", models$lasso)
#print(fitted_model)
#m_predict.lasso(fitted_model, modelData)
#fitted_model$lambda_list <-NULL
#fitted_model_1 <- structure(fitted_model, class="hello")
...
goodnessRankResults = data.frame(matrix(ncol=0, nrow=length(fitResults)))
...
for (i in 1:length(goodness_fit_metrics)){
  ...
  selectData[which(selectData$model_labels == m_name), g] <- m[[g]]
  }
