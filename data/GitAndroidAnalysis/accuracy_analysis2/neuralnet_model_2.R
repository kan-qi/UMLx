...
predictAndMeasure <- function(model, pcaComp, x_train, y_train, x_test, y_test, timer, grid=NULL, verbose=FALSE) {
...
fitModel <- function(data.train, verbose=FALSE) {
...
neuralnet_model <- function(dataset, verbose=FALSE) {
  ...
  if (verbose) {
    cat("Dimensions used for training...\n")
    print(names(data))
  }
  ...
  for(pcaComp in seq(6, 10)) {
    ...
    if ((nrow(perf_grid) == 1) || (RMSE < min(perf_grid$RMSE.test))) {
      hyperparameters <- c("pcaComp" = pcaComp, "bestTune" = list(nnet_model$bestTune))
    }
  }
...
plot.nnet<-function(mod.in,nid=TRUE,all.out=TRUE,all.in=TRUE,bias=TRUE,wts.only=FALSE,rel.rsc=5,
                    circle.cex=5,node.labs=TRUE,var.labs=TRUE,x.lab=NULL,y.lab=NULL,
                    line.stag=NULL,struct=NULL,cex.val=1,alpha.val=1,
                    circle.col='lightblue',pos.col='black',neg.col='grey',
                    bord.col='lightblue', max.sp=FALSE,...){
  ...
  nnet.vals<-function(mod.in,nid,rel.rsc,struct.out=struct){
    ...
    if('mlp' %in% class(mod.in)){
      ...
      assign('bias',FALSE,envir=environment(nnet.vals))
    }
  ...
  #initiate plot
  plot(x.range,y.range,type='n',axes=FALSE,ylab='',xlab='',...)
  ...
  layer.lines<-function(mod.in,h.layer,layer1=1,layer2=2,out.layer=FALSE,nid,rel.rsc,all.in,pos.col,
                        neg.col,...){
  ...
  for(lay in lays){
    for(node in 1:struct[lay[1]]){
      layer.lines(mod.in,node,layer1=lay[1],layer2=lay[2],nid=nid,rel.rsc=rel.rsc,all.in=TRUE,
                  pos.col=alpha(pos.col,alpha.val),neg.col=alpha(neg.col,alpha.val))
    }
  }
  ...
  if(is.logical(all.out))
    ...
    layer.lines(mod.in,node.in,layer1=length(struct)-1,layer2=length(struct),out.layer=TRUE,nid=nid,rel.rsc=rel.rsc,
