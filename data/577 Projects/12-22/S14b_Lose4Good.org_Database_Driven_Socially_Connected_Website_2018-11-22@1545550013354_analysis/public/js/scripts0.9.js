function model_file_upload_fnc() {
	...
	$.ajax({
		...
		success : function(response) {
			...
			$("#robustness-diagrams-block").append(response);
		},
		error : function() {
...
function query_model_statistics_func(){
	var modelId = $(this).closest('.list-item').data('model-id');
//	console.log(modelId);
//	var url = $(this).attr("href");
//	console.log(url);
	console.log('query_model_statistics_func');
	$.ajax({
		type : 'GET',
		url : 'queryModelStatistics?model_id='+modelId,
		success : function(response) {
//			console.log(response);
			$("#display-panel").html("");
			$("#display-panel").append(response);
...
function query_graph_detail_func(){
	var url = $(this).attr("href");
	console.log(url);
	$.ajax({
		type : 'GET',
		...
		success : function(response) {
			console.log(response);
			$("#diagram-detail-panel").html("");
			$("#diagram-detail-panel").append(response);
		},
		error : function() {
			console.log("fail");
			alert("There was an error submitting comment");
...
function query_repo_statistics_func(){
	$.ajax({
		type : 'GET',
		url : "queryRepoAnalytics",
		success : function(response) {
			console.log(response);
			$("#display-panel").html("");
			$("#display-panel").append(response);
...
//function drawChartByHighCharts(){
//	Highcharts.chart('repository-stats-chart', {
//	    chart: {
//	        type: 'scatter',
//	        zoomType: 'xy'
//	    },
...
//function drawBarChartBySVG(data1, elementSelector){
//
...
//	$('#display-panel').html("");
//	$('#display-panel').append('<div class="block panel panel-default"><div class="block-title panel-heading">Repository Statistics</div><div id="repository-stats-chart" class="panel-body"></div></div>');
...
//var svg = d3.select("#repository-stats-chart").append("svg")
//    .attr("width", width + margin.left + margin.right)
//    .attr("height", height + margin.top + margin.bottom)
//  .append("g")
...
//var tooltip = d3.select("#repository-stats-chart").append("div")
//    .attr("class", "tooltip")
//    .style("opacity", 0);
//
//
...
//
//for (var k = 0; k < 100; k++) {
//lineData.push({x: 0.01 * k, y: a * Math.pow(b, 0.01 * k)});
//}
//
...
//
//// load data
//d3.csv("output/model-statistics.csv", function(error, data) {
...
$('#repository-stats-chart').append('<div id="model-selection-panel"> \
...
$(document).ready(function() {
	$(document).on('click','a.diagram-title', query_graph_detail_func);
	$(document).on('click','a.model-title', query_model_detail_func);
	$(document).on('click','#repo-statistics', query_repo_statistics_func);
	$(document).on('click','.btn.model-statistics', query_model_statistics_func);
