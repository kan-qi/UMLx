...
<<<<<<< HEAD
    ...
    // query data info from mongodb
    function queryAPIRequest(apiId,apiDate,apiRequestTime,callbackfunc){
        MongoClient.connect(url,function (err,db) {
            if (err) throw err;
            console.log("query api info: start collect")

            // data insertion example
            // [ { ip: '127.345.234', request_time: '12:30', date: '2020-1-3' },
            //   { ip: '127.345.234', request_time: '12:50', date: '2020-1-3' }]
            db.db("apiInfo").collection("apiInfo").find({},{_id:0}).toArray(function (err, result) {
                ...
                callbackfunc(apiInfo);
            });
        });
    }

    // pages review
    function queryPageReview(pageId,pageDate,callbackfunc){
        MongoClient.connect(url,function (err,db) {
            if (err) throw err;
            console.log("query page review info: start collect")
            // data insertion example
            // [ { Id: 1, date: '2020-1-3' },
            //   { Id: 2, date: '2020-1-4' }]
            db.db("pageInfo").collection("pageInfo").find({},{_id:0}).toArray(function (err, result) {
...
//        saveEffortEstimationQueryResult:saveEffortEstimationQueryResult,
        ...
        queryPageReview:queryPageReview
    }
